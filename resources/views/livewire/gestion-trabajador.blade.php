                            ...
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-800 bg-white text-sm">
                                    <div class="flex items-center justify-center">
                                        <button type="button" wire:click="setId({{$trabajador['id']}},{{1}})"
                                                class="bg-gray-200 p-3 rounded hover:bg-red-500">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                 viewBox="0 0 24 24"
                                                 class="fill-current text-black">
                                                <path
                                                    d="M10 15.586L6.707 12.293 5.293 13.707 10 18.414 19.707 8.707 18.293 7.293z"></path>
                                            </svg>
                                        </button>
                                        <button type="button" wire:click="setId({{$trabajador['id']}},{{2}})"
                                                class="bg-gray-200 p-3 rounded hover:bg-red-500">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                 viewBox="0 0 24 24"
                                                 class="fill-current text-black">
                                                <path
                                                    d="M16.192 6.344L11.949 10.586 7.707 6.344 6.293 7.758 10.535 12 6.293 16.242 7.707 17.656 11.949 13.414 16.192 17.656 17.606 16.242 13.364 12 17.606 7.758z"></path>
                                            </svg>
                                        </button>

                                        <button type="button" wire:click="setId({{$trabajador['id']}},{{3}})"
                            ...
                            @if($currentuser!=null)
                                <div class="flex-row justify-center">
                                    <div class="flex justify-center m-2">
                                        <img class="w-1/6" src="{{$currentuser['img_perfil']}}">
                                    </div>
                                    <p class="text-gray-900 text-center">{{ $currentuser['nombre'] }}</p>
                                    <p class="text-gray-900 text-center"><span class="font-bold">CI:</span> {{ $currentuser['ci'] }}</p>

                                    <p class="text-gray-900 text-center"><span class="font-bold">Dirección:</span> {{ $currentuser['direccion'] }}</p>
                                    <p class="text-gray-900 text-center"><span class="font-bold">Teléfono:</span> {{ $currentuser['telefono'] }}</p>
                                    <p class="text-gray-900 text-center"><span class="font-bold">Correo:</span> {{ $currentuser['email'] }}</p>
                                    <p class="text-gray-900 text-center"><span class="font-bold">Sexo:</span> {{ $currentuser['sexo'] }}</p>
                                    <p class="text-gray-900 text-center  text-xl font-bold mt-5 uppercase">Servicios</p>

                                    @foreach($currentuser['servicios'] as $servicio)
                                        @php
